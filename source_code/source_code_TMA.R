getOrders <- function(store, newRowList, currentPos, info, params) {
  ...
  if (store$iter > params$lookbacks$long) {
    for (i in params$series) {
      if(i !=0){
        #get close price
        close_price = store$cl[[i]]
        
        #get current_close from store
        current_close = newRowList[[i]]$Close
        
        startIndex <-  store$iter - (params$lookbacks$long+1)
        
        #calculate TMA
        tma = getTMA(close_price[seq(startIndex, store$iter)], params$lookbacks)
        
        #calculate pos
        pos[i] = getPosSignFromTMA(tma) * getPosSize(current_close)
...
getTMA <- function(prices, lookbacks, with_checks=FALSE) {
  ...
  for (i in 1 : length(lookbacks)){
    ...
    #calculate SMA for each lookbakcs

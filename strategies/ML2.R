...
getOrders <- function(store, newRowList, currentPos, info, params){
  ...
  model <- rpart(Close ~ Close_1 + Close_2 + Close_3 + Close_4, data = lagged_data)
  
  # Create a new data frame with the previous 3 closing prices for today
  if (nrow(df) >= 4){
  new_data <- data.frame(Close_1 = df$Close[length(df$Close)-3],
                         Close_2 = df$Close[length(df$Close)-2],
                         Close_3 = df$Close[length(df$Close)-1],
                         Close_4 = df$Close[length(df$Close)])

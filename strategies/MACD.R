...
getOrders <- function(store, newRowList, currentPos, info, params){
  ...
  if (store$iter > params$nSlow) {
    startIndex <-  store$iter - params$nFast
    for (i in 1:length(params$series)) {
      
      #for(d in 1: length(newRowList[[params$series[i]]])-1){
      # r <- vector(mode = "numeric", length = length(newRowList[[params$series[i]]])-1)
      # r[d]  <- newRowList[[params$series[i]]]$Close
      
      # }
      cl <- newRowList[[params$series[i]]]$Close
      
      macd_data <- MACD(store$cl[startIndex:store$iter,i], nFast= 12, nSlow = 26,nSig =9)
    }
  }

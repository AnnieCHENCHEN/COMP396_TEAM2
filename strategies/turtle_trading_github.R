...
try(rm("portfolio","account","N","symbol","symbols","ClosePrice","CurrentDate","equity","Units",
       "maxUnits","size","Stop","equity","TxnPrice","initDate","initEq","Posn","verbose"),silent=TRUE)
...
# @todo: decrease the size of the notional account by 20% each time lose 10% of original account (10% drawdown).  
# E.g., if trading a $1M account and down 100K, trade as if $800K account until out of drawdown.  
# If lose another 10% from 800K, or 80K loss, then reduce account size another 20% for notional size of 640K.

# Set up indicators 建立指标
...
for(symbol in symbols){
  ...
  x$N <- ATR(x[,c(2,3,4)], n=20, maType=EMA, wilder=TRUE)[,'atr']   #True Range / Average True Range
  assign(symbol,x) # 将x的值赋值给symbol
}
...
#115-188代码 turtle trading 核心 ----getOrders
# Create trades
for( i in 57:NROW(x) ) { # Assumes all dates are the same
  ...
  for(symbol in symbols){
    ...
    UnitSize = as.numeric(trunc((size * equity)/(x[i-1,'N']*ClosePrice)))  # 单位仓 （存疑！！！！！！）
    
    # Position Entry (assume fill at close, so account for slippage)
    if( Posn == 0 ) { 
      # Initiate Long position  买的建仓信号 (用high price Hi)
      if( as.numeric(Hi(x[i-1,])) > as.numeric(x[i-2,'Max55']) ) { 
        ...
        # Initiate Short position  卖的建仓信号 (用low price Lo)
      ...
      # Position exits and stops #动态止损和止盈

    ...
    for(i in 1:length(params$series)) {
      ...
      if(i == 6 || i == 10){
        ...
        if(store$Hi[store$iter,i] > storeOfEnEx$Max_En2[store$iter-1,i]){
          ...
          StopPrice_S = TxnPrice_S - params$Multi_N * N_short #?˳?/ֹ???۸?
          StopPrice_S2 = TxnPrice_S - (params$Multi_N-3) * N_short #ƽ???ֲֵļ۸?
          
          # Add to SHORT position   
          if(store$Hi[store$iter,i] > ( TxnPrice_S + N_short * (params$Multi_N-2) )) {
            
            pos_short[params$series[i]] = -2 * UnitSize #?Ӳ?
          }
        }else{
          ...
          if(as.numeric(store$Lo[store$iter,i]) < as.numeric(storeOfEnEx$Min_En2[store$iter-1,i])){
            ...
            StopPrice_L = TxnPrice_L + params$Multi_N*N_long #?˳?/ֹ???۸?
            StopPrice_L2 = TxnPrice_L + (params$Multi_N-3)*N_long #ƽ???ֲֵļ۸?
            
            # Add to LONG position
            if(store$Lo[store$iter,i] < ( TxnPrice_L - N_long*(params$Multi_N-2) )) {
              
              pos_long[params$series[i]] = 2 * UnitSize#?Ӳ?
            }
          }
        }
